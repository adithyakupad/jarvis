...
function ContextReplanForm({ presentation, busy, error, onBack, onInvalid, onSubmit }: { presentation: RunPresentation; busy: boolean; error: string; onBack: () => void; onInvalid: () => void; onSubmit: (packet: ReturnType<typeof ContextPacketSchema.parse>) => Promise<void> }): ReactNode {
  ...
  return <form className="active-object modify-object" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const packet = ContextPacketSchema.safeParse({ context: data.get("context"), expectedBehavior: data.get("expectedBehavior"), actualBehavior: data.get("actualBehavior"), reproductionSteps: String(data.get("reproductionSteps") ?? "").split("\n"), evidence: data.get("evidence"), constraints: String(data.get("constraints") ?? "").split("\n") }); if (!packet.success) { onInvalid(); return; } void onSubmit(packet.data); }}>
    ...
    <p>Describe the missing fact naturally. JARVIS will assess it against the repository before replanning.</p>
    <label htmlFor="freeform-context">What should JARVIS know?<textarea id="freeform-context" name="context" rows={4} defaultValue={existing?.context ?? ""} placeholder="My suit starts freezing at high altitudes when I fly." autoFocus /></label>
