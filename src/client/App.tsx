...
function WorkspaceView({ project, onRunDetails }: { project: Project; onRunDetails: () => void }): ReactNode {
  ...
  function center(): ReactNode {
    if (!activeRun) return <article className="active-object instruction-object"><p className="eyebrow">Task / read-only planning first</p><h2>What should JARVIS change?</h2><p>Enter today's coding task here. JARVIS asks the selected provider to inspect the repository and propose a bounded plan; no files change until you approve it.</p><label htmlFor="instruction">Task instruction</label><textarea id="instruction" placeholder="Add a multiply function and an automated test. Follow existing conventions. Do not commit or push." value={instruction} onChange={(event) => setInstruction(event.target.value)} rows={5} />{error && <p className="inline-error" role="alert">{error}</p>}<div className="instruction-footer"><span><i aria-hidden="true" /> Planning is read-only</span><button className="button primary" onClick={() => void invoke(() => service.inspect(project.id, instruction))}>Inspect and propose</button></div></article>;
...
function ValidationEvidence({ verification }: { verification: NonNullable<RunPresentation["run"]["verification"]> }): ReactNode {
  ...
  if (!result) return <section className="surface"><div className="section-kicker">Validation</div><p>{verification.message}</p></section>;
  const title = {
    not_supported: "No supported automated tests detected",
