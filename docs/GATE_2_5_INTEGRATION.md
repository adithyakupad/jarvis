...
A Context Packet stores external facts that repository inspection cannot establish. Its canonical freeform field is `summary`; structured `expectedBehavior`, `actualBehavior`, `reproductionSteps`, `evidence`, and `constraints` remain optional. Legacy `context` and `problem` fields remain readable for existing persisted packets. At least one normalized non-empty field is required. Packets are associated with a planning run and restored through both run reads and project `activeRun` restoration.
