...
`npm run jarvis` creates `<JARVIS_DATA_DIR>/jarvis.instance.lock` (or `data/jarvis.instance.lock` by default) containing safe process and compatibility metadata. Ctrl+C or SIGTERM starts one bounded graceful-shutdown transaction: JARVIS closes Fastify, closes its owned resources, verifies the complete lock ownership record, and removes the lock only when it still belongs to that runtime instance. Repeated signals do not start competing cleanup work. A compatible duplicate invocation prints the existing URL and does not start another frontend/API pair. Dead locks are recovered, so stale-lock recovery remains a fallback after crashes, forced termination, power loss, or other situations where graceful cleanup cannot run. Malformed locks are preserved with a diagnostic suffix, and JARVIS never deletes a lock owned by another active instance. A live PID is never trusted without probing `/api/health`, which protects against PID reuse.
